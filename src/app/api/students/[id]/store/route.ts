...
import { MOCK_STORE } from "@/lib/api/mockStudent";
...
export async function GET(_request: Request, { params }: Props) {
  ...
  if (USE_MOCK) {
    return NextResponse.json(MOCK_STORE);
  }

  return parentLaravelGet(`/store/items?student_id=${encodeURIComponent(id)}`, 12_000);
}
